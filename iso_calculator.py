...
iso_values = [100, 125, 160, 200, 250, 320, 400, 500, 640, 800,
              1000, 1250, 1600, 2000, 2500, 3200, 4000, 5000, 6400,
              8000, 10000, 12800, 16000, 20000, 25600]

apertures = [1.4, 2, 2.8, 4, 5.6, 8, 11, 16, 22]
shutter_speeds = [1, 1/2, 1/4, 1/8, 1/15, 1/30, 1/60,
                  1/125, 1/250, 1/500, 1/1000, 1/2000, 1/4000, 1/8000]
shutter_speed_labels = ["1s", "1/2s", "1/4s", "1/8s", "1/15s", "1/30s",
                        "1/60s", "1/125s", "1/250s", "1/500s", "1/1000s", "1/2000s", "1/4000s", "1/8000s"]

shutter_speed_options = list(zip(shutter_speeds, shutter_speed_labels))

lighting_conditions = {
    16: "Snow/Sand", 15: "Sunny", 14: "Slight Overcast", 
    13: "Overcast", 12: "Heavy Overcast", 11: "Open Shade/Sunset"
}
ev_options = [(ev, f"EV {ev}: {lighting_conditions[ev]}") for ev in sorted(lighting_conditions.keys(), reverse=True)]
...
def calculate_variable():
    ...
    data = {
        ...
        'shutterspeed': float(request.form.get('shutterspeed', defaults['shutterspeed'])),
    ...
    if request.method == 'POST':
        locks = [data['lock_aperture'], data['lock_shutter_speed'], data['lock_iso']]
        if sum(locks) != 2:
            data['error'] = "Please lock exactly two variables to calculate the third."
        else:
            try:
                if not data['lock_aperture']:
                    calculated_aperture = math.sqrt((data['iso'] / 100) * (2 ** data['ev']) * data['shutterspeed'])
                    if calculated_aperture < min(apertures) or calculated_aperture > max(apertures):
                        data['warning'] = "Calculated aperture is out of range."
                    else:
                        data['result'] = find_nearest(apertures, calculated_aperture)
                        data['result_key'] = 'Aperture'
                elif not data['lock_shutter_speed']:
                    calculated_speed = ((data['aperture'] ** 2) / ((2 ** data['ev']) * (data['iso'] / 100)))
                    if calculated_speed < min(shutter_speeds) or calculated_speed > max(shutter_speeds):
                        data['warning'] = "Calculated shutter speed is out of range."
                    else:
                        data['result'] = to_fraction(find_nearest(shutter_speeds, calculated_speed))
                        data['result_key'] = 'Shutter Speed'
                elif not data['lock_iso']:
                    if data['shutterspeed'] == 0:
                        raise ValueError("Shutter speed must not be zero when calculating ISO.")
                    calculated_iso = 100 * ((data['aperture'] ** 2) / data['shutterspeed']) / (2 ** data['ev'])
                    if calculated_iso < min(iso_values) or calculated_iso > max(iso_values):
                        data['warning'] = "Calculated ISO is out of range."
                    else:
                        data['result'] = find_nearest(iso_values, calculated_iso)
                        data['result_key'] = 'ISO'
            except ValueError as e:
                data['error'] = f"Invalid input: {str(e)}"

    return render_template_string(HTML_TEMPLATE, **data, apertures=apertures, shutter_speed_options=shutter_speed_options,
                                  iso_values=iso_values, ev_options=ev_options)
